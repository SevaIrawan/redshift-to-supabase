...
# Configuration
SOURCE_TABLE = "blue_whale_sgd"
TARGET_TABLE = "rs_blue_whale_sgd"
BATCH_SIZE = 2000
SOURCE_SCHEMA = "public"
TARGET_SCHEMA = "public"

print("=" * 70)
print("REDSHIFT TO SUPABASE MIGRATION - SGD")
print("=" * 70)
...
try:
    cursor = supabase.cursor()
    cursor.execute(f'ALTER TABLE {TARGET_SCHEMA}."{TARGET_TABLE}" DROP CONSTRAINT IF EXISTS rs_blue_whale_sgd_unique_key_unique')
    supabase.commit()
    cursor.close()
...
try:
    cursor = supabase.cursor()
    cursor.execute(f'ALTER TABLE {TARGET_SCHEMA}."{TARGET_TABLE}" ADD CONSTRAINT rs_blue_whale_sgd_unique_key_unique UNIQUE (unique_key)')
    supabase.commit()
    cursor.close()
